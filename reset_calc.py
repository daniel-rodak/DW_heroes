...
def get_report(values):
    ...
    dfl = df.melt(NAME_COL, value_name="Dmg").drop(columns="variable")
    dfl[NAME_COL] = dfl[NAME_COL].str.strip()
    dfl['Dmg'] = dfl['Dmg'].astype(float)
    display(dfl.groupby(NAME_COL).agg({'Dmg': ['mean', 'std']}))
    ...
    display(pd.DataFrame(res, columns=[NAME_COL, 'Reset']))
...
ranges = {
    "ResetCalcExample!A1:K11": "ABealinda",
}

for r in ranges.keys():
    print(f"##### Arkusz: {ranges[r]} #####")
    result = sheet.values().get(spreadsheetId=SSID, range=r).execute()
    values = result.get('values', [])
    kde, sol, dfl = get_report(values)
